...
pub struct CarbonIIOxideChartProps {
...
pub fn carbon_ii_oxide_chart(props: &CarbonIIOxideChartProps) -> Html {
        ...
        spawn_local(async move {
            match get_air_quality_data().await {
                Ok(fetched_data) => {
                    ...
                    log::info!("Filtered data for CO chart: {} records", filtered_data.len());
                    ...
                    let series_co = build_series(&filtered_data,
                        |record| record.co,
                        |record| parse_timestamp(&record.timestamp).unwrap()
                    );

                    // If series is empty after filtering, show no data
                    if series_co.is_empty() {
                        log::warn!("No CO data available for the selected time range");
                        chart_config.set(None);
                        return;
                    }

                    let x_range = compute_time_range(&series_co);
    ...
    <div class="chart-wrapper">
        {
            if let Some(config) = &*chart_config {
                ...
                html! { <div class="chart-loading">{ "Loading Carbon (II) Oxide data..." }</div> }
            }
        }
    </div>
    }
