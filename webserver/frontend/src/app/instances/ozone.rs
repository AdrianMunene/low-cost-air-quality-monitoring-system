            ...
            match get_air_quality_data().await {
                Ok(fetched_data) => {
                    ...
                    let (x_min, x_max) = if !series_o3.is_empty() {
                        (series_o3.first().unwrap().timestamp, series_o3.last().unwrap().timestamp)
                    } else {
                        (Utc::now(), Utc::now())
                    };

                    // Determine the y-axis range
                    let y_min = series_o3.iter().map(|p| p.value).fold(f64::INFINITY, f64::min);
                    let y_max = series_o3.iter().map(|p| p.value).fold(f64::NEG_INFINITY, f64::max);
                    // Default range for O₃ (0-100 ppb is a typical range for ambient monitoring)
